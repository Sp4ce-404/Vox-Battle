...
export default class RecordPanel {
  ...
  init() {
    // 1. Enable mic permissions flow
    this.enableMicBtn.addEventListener("click", () => this.requestMicPermission());
    
    // 2. Record button handler
    this.recordBtn.addEventListener("click", () => this.toggleRecording());
    
    // 3. Hear myself monitoring handler
    this.hearMyselfToggle.addEventListener("change", (e) => {
      this.engine.setHearMyself(e.target.checked);
    });
    
    // 4. Playback audio player listener
  ...
  downloadAudio() {
    ...
    const filename = `voxbattle_${Date.now()}.${ext}`;
  ...
  async shareAudio() {
    ...
    const ext = this.recordedBlob.type.includes("ogg") ? "ogg" : "webm";
    const file = new File([this.recordedBlob], `voxbattle_${Date.now()}.${ext}`, {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
          title: "My VoxBattle Voice Clip",
          text: "Check out my game voice preset from VoxBattle!"
        });
