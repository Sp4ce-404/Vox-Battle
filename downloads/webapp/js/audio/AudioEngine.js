...
export default class AudioEngine {
  constructor() {
    ...
    this.analyser = null;
    this.noiseGate = null;
    this.noiseGateThreshold = -50; // dB — signals below this are silenced
  ...
  async init() {
    ...
    this.context = new AudioContextClass({
      ...
      sampleRate: 44100 // lock to 44.1kHz to avoid unnecessary resampling overhead
    });
    ...
    this.analyser.fftSize = 128; // smaller FFT = less CPU per frame
    this.analyser.smoothingTimeConstant = 0.6;
    
    // Noise gate: DynamicsCompressor configured as a gate
    // This strips low-level synthesis artifacts and background hiss
    this.noiseGate = this.context.createDynamicsCompressor();
    this.noiseGate.threshold.setValueAtTime(this.noiseGateThreshold, this.context.currentTime);
    this.noiseGate.knee.setValueAtTime(2, this.context.currentTime);
    this.noiseGate.ratio.setValueAtTime(20, this.context.currentTime); // very high ratio = gate behavior
    this.noiseGate.attack.setValueAtTime(0.002, this.context.currentTime);
    this.noiseGate.release.setValueAtTime(0.05, this.context.currentTime);
    ...
    this.effects.compressor.output.connect(this.noiseGate);
    this.noiseGate.connect(this.outputGain);
